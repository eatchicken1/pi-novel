import { BookOpen, FileText, FolderOpen, RefreshCw, Search, X } from "lucide-react";
...
export function LibraryPage({ overview, onOpenProject, onRescan, onOpenWorkspaceSetup }: LibraryPageProps) {
	...
	const projects = useMemo(() => (overview?.projects ?? []).filter((project) => `${project.title} ${project.projectId}`.toLowerCase().includes(query.toLowerCase())), [overview?.projects, query]);
	...
	function clearFilters(): void { setQuery(""); }
	...
	return <div className="page-stack"><section className="page-heading"><div><p className="eyebrow">LIBRARY</p><h1>作品库</h1><p>按最近编辑继续你的故事。</p></div><button className="quiet-button" type="button" onClick={rescan} disabled={isScanning}><RefreshCw size={15} className={isScanning ? "spin" : ""} /> {isScanning ? "扫描中" : "重新扫描"}</button></section>{scanError && <div className="inline-alert" role="alert"><RefreshCw size={14} /> {scanError}</div>}<div className="library-toolbar"><div className="search-box"><Search size={16} /><input aria-label="搜索项目名称或 ID" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索项目名称或 ID" />{query && <button className="search-clear" type="button" aria-label="清除搜索" onClick={() => setQuery("")}><X size={14} /></button>}</div></div><div className="library-summary"><span><BookOpen size={15} /> {projects.length} 个项目</span></div><div className="library-grid">{projects.map((project) => <button className="library-project" type="button" key={project.projectId} aria-label={`打开项目 ${project.title}`} onClick={() => onOpenProject(project)}><div className="library-project-top"><div className="project-mark"><FolderOpen size={18} /></div><span className="status-pill neutral">{project.status === "needs_migration" ? "待处理" : "已就绪"}</span></div><h2>{project.title}</h2><p>{project.wordCount.toLocaleString("zh-CN")} 字 · {project.kind === "legacy" ? "只读" : "可编辑"}</p><div className="library-project-footer"><span>{project.status === "needs_migration" ? "需要迁移评估" : "继续创作"}</span><span>{formatProjectDate(project.lastModifiedAt)}</span></div></button>)}{projects.length === 0 && <div className="empty-card wide"><BookOpen size={20} /><div><strong>没有匹配的项目</strong><p>调整搜索条件，或将项目目录加入 Workspace 后重新扫描。</p>{query && <button className="text-button" type="button" onClick={clearFilters}>清除搜索 <X size={13} /></button>}</div></div>}</div></div>;
}

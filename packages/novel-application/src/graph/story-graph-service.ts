...
export class StoryGraphService {
	...
	async getGraph(projectId: string, query: StoryGraphQuery = {}): Promise<StoryGraph> {
		...
		const full: StoryGraph = { projectId, nodes, edges, sourceHash: sources.sourceHash, generatedAt: new Date().toISOString() };
		repository.replaceAll(full);
		// 过滤查询直接走持久化投影
		const filtered = repository.query(projectId, query);
		return { projectId, nodes: filtered.nodes, edges: filtered.edges, sourceHash: sources.sourceHash, generatedAt: full.generatedAt };
	}

	private buildNodes(sources: { events: Array<{ eventId: number; chapter: number; action: string }>; characters: Array<{ characterId: string; label: string | null }>; clues: Array<{ clueId: string; label: string | null; chapter: number | null }>; claims: Array<{ claimId: string; label: string | null; revealChapter: number | null }>; promises: Array<{ promiseId: string; label: string | null }> }) {
		const nodes: StoryGraph["nodes"] = [];
		for (const event of sources.events) {
			nodes.push({ nodeId: "event-" + event.eventId, type: "event", ref: String(event.eventId), label: event.action.slice(0, 60), chapter: event.chapter, meta: {} });
		}
		for (const character of sources.characters) {
			nodes.push({ nodeId: "character-" + character.characterId, type: "character", ref: character.characterId, label: character.label, chapter: null, meta: {} });
		}
		for (const clue of sources.clues) {
			nodes.push({ nodeId: "clue-" + clue.clueId, type: "clue", ref: clue.clueId, label: clue.label, chapter: clue.chapter, meta: {} });
		}
		for (const claim of sources.claims) {
			nodes.push({ nodeId: "claim-" + claim.claimId, type: "claim", ref: claim.claimId, label: claim.label, chapter: claim.revealChapter, meta: {} });
		}
		for (const promise of sources.promises) {
			nodes.push({ nodeId: "promise-" + promise.promiseId, type: "promise", ref: promise.promiseId, label: promise.label, chapter: null, meta: {} });
		}
		return nodes;
	}

	private buildEdges(sources: { events: Array<{ eventId: number; characterRefs: string[]; clueRefs: string[]; claimRefs: string[]; causes: number[] }> }, nodes: StoryGraph["nodes"]) {
		const nodeIds = new Set(nodes.map((node) => node.nodeId));
		const edges: StoryGraph["edges"] = [];
		let edgeCounter = 0;
		const push = (sourceNodeId: string, targetNodeId: string, type: StoryGraph["edges"][number]["type"], label: string | null) => {
		...
		for (const event of sources.events) {
			...
			for (const characterId of event.characterRefs) push("event-" + event.eventId, "character-" + characterId, "involves", null);

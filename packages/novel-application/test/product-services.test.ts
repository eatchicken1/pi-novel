...
function createEnv(): Env {
	...
	const project: ProjectRecord = {
		projectId: "p-1", title: "T", rootPath: projectRoot, kind: "native", status: "ready", wordCount: 20, lastModifiedAt: new Date().toISOString(), manifest: null,
	};
	...
	const engine = {
		getStatus: async () => ({ nextChapter: 3, finalizedChapters: [1, 2], memoryStatus: "current", continuityStatus: "ok", openThreads: 1, overdueThreads: 0, unresolvedSetups: 0, downstreamReviewRequired: false, currentMovement: "m2" }),
		listChapters: async () => [
			{ chapter: 1, title: "一", wordCount: 4, contentHash: "a", revision: 1, finalized: true, updatedAt: "2026-08-16T00:00:00.000Z" },
			{ chapter: 2, title: "二", wordCount: 5, contentHash: "b", revision: 1, finalized: true, updatedAt: "2026-08-16T00:00:00.000Z" },
		],
		readChapter: async () => ({ projectId: "p-1", chapter: 1, title: "一", text: "正文内容。", contentHash: "a", revision: 1, updatedAt: "2026-08-16T00:00:00.000Z" }),
		analyzeRevisionImpact: async () => ({ severity: "safe-local", affectedChapters: [], affectedCharacters: [], affectedThreads: [], affectedClues: [], affectedPromises: [], summary: "s", analyzedAt: "2026-08-16T00:00:00.000Z" }),
	...
	return {
		projectRoot, registry, workspace, reviewSources, engine,
		cleanup() { registry.closeAll(); rmSync(projectRoot, { recursive: true, force: true }); },
		setGraphSources(value: unknown) { state.graphSources = value; },
	};
}
...
describe("product services", () => {
	it("R1-R2/R5: review projection separates severity from blocking and returns landing chapters", async () => {
		...
		try {
			env.reviewSources.push({
				sourceCode: "REALIZED_REVEAL_BEFORE_PROOF", severity: "error", priority: "P0", scope: "future-chapter", repairScope: "event-graph", blockingForCurrentAction: false, chapter: null, scene: null, landingChapter: 5, message: "TC4 揭示早于证明", evidence: null,
			});
			const service = new ReviewService({ workspace: env.workspace, engine: env.engine as unknown as NovelEnginePort, registry: env.registry, id: { id: () => "iss-" + Math.random() } });
	...
	it("R4: refresh marks disappeared sources resolved automatically", async () => {
		const env = createEnv();
		try {
			env.reviewSources.push({ sourceCode: "OLD_ISSUE", severity: "warning", priority: "P2", scope: "chapter", repairScope: "prose", blockingForCurrentAction: false, chapter: 1, scene: null, landingChapter: null, message: "x", evidence: null });
			const service = new ReviewService({ workspace: env.workspace, engine: env.engine as unknown as NovelEnginePort, registry: env.registry, id: { id: () => "iss-" + Math.random() } });
			await service.refresh("p-1");
			expect((await service.list("p-1")).issues).toHaveLength(1);
	...
	it("G1-G4: story graph projection derives nodes/edges with chapter and type filters", async () => {
		...
		try {
			env.setGraphSources({
				events: [
					{ eventId: 1, chapter: 1, action: "接案", causes: [], characterRefs: ["heroine"], clueRefs: ["CL1"], claimRefs: [], professionalActionRefs: [], marriageRefs: [], irreversible: false },
					{ eventId: 2, chapter: 2, action: "揭示", causes: [1], characterRefs: [], clueRefs: [], claimRefs: ["T1"], professionalActionRefs: [], marriageRefs: [], irreversible: false },
				],
	...
	it("G5: engine source change rebuilds the projection (stale rebuild)", async () => {
		const env = createEnv();
		try {
			env.setGraphSources({ events: [{ eventId: 1, chapter: 1, action: "a", causes: [], characterRefs: [], clueRefs: [], claimRefs: [], professionalActionRefs: [], marriageRefs: [], irreversible: false }], characters: [], clues: [], claims: [], promises: [], sourceHash: "h1" });
			const service = new StoryGraphService(env.workspace, env.engine as unknown as NovelEnginePort, env.registry);
			const first = await service.getGraph("p-1");
			expect(first.nodes).toHaveLength(1);
			env.setGraphSources({ events: [{ eventId: 1, chapter: 1, action: "a", causes: [], characterRefs: [], clueRefs: [], claimRefs: [], professionalActionRefs: [], marriageRefs: [], irreversible: false }, { eventId: 2, chapter: 2, action: "b", causes: [], characterRefs: [], clueRefs: [], claimRefs: [], professionalActionRefs: [], marriageRefs: [], irreversible: false }], characters: [], clues: [], claims: [], promises: [], sourceHash: "h2" });
	...
	it("S1-S3: studio snapshot contains UI-needed data only and shows pending change sets", async () => {
		...
		try {
			...
			const review = new ReviewService({ workspace: env.workspace, engine: env.engine as unknown as NovelEnginePort, registry: env.registry, id: { id: () => "iss-" + Math.random() } });
			const changeSets = new ChangeSetService({ workspace: env.workspace, engine: env.engine as unknown as NovelEnginePort, registry: env.registry, files: new FileTransaction(), idempotency: { hasResult: () => false, storeResult: () => undefined }, id: { id: () => "cs-" + Math.random() }, clock: { now: () => new Date().toISOString() } });
			await changeSets.create({ projectId: "p-1", title: "pending edit", kind: "content", source: "agent", intent: "x", baseRevision: null, operations: [{ operationId: "op-1", kind: "replace-text", target: "manuscript/chapter-001.md", startChar: 0, endChar: 2, text: "新" }] });
			const tasks = { listTasks: () => [], createTask: () => undefined, updateTask: () => undefined, getTask: () => null, appendEvent: () => undefined, listEvents: () => [], maxSequence: () => 0, createAgentRun: () => undefined, updateAgentRun: () => undefined, getAgentRun: () => null } as unknown as TaskRepositoryPort;
			const studio = new StudioService({ workspace: env.workspace, reads, review, registry: env.registry, tasks, engine: env.engine as unknown as NovelEnginePort });
	...
	it("T1-T6: task lifecycle, cancellation, failed, event order, Last-Event-ID resume and idempotent retry", async () => {
		...
		try {
			...
			const runtime = {
				startTask: async (input: { taskId: string }) => { startedRuns.push(input.taskId); },
				cancelTask: () => undefined,
				subscribe: () => () => undefined,
			} as unknown as AgentRuntimePort;
			const repository = {
				createTask: () => undefined, updateTask: () => undefined,
				getTask: () => null, listTasks: () => [],
				appendEvent: () => undefined, listEvents: () => [], maxSequence: () => 0,
				createAgentRun: () => undefined, updateAgentRun: () => undefined, getAgentRun: () => null,
			} as unknown as TaskRepositoryPort;
			const service = new TaskService(repository, runtime, { now: () => new Date().toISOString() });
			const task = await service.create({ projectId: "p-1", forgeSessionId: null, type: "generate-chapter", intent: "write ch3", modelId: "faux-1" }, env.projectRoot, "key-t1");
			...
			const repositoryWithState = {
				createTask: () => undefined, updateTask: () => undefined,
				getTask: (taskId: string) => tasks.includes(taskId) ? { taskId } as never : null, listTasks: () => [],
				appendEvent: () => undefined, listEvents: () => [], maxSequence: () => 0,
				createAgentRun: () => undefined, updateAgentRun: () => undefined, getAgentRun: () => null,
			} as unknown as TaskRepositoryPort;
	...
	it("H1: history service lists and reads commit records", async () => {
		...
		try {
			...
			commits.record({ commitId: "cm-1", projectId: "p-1", changeSetId: "cs-1", actor: "user", summary: "edit", affectedFiles: ["a.md"], beforeHashes: {}, afterHashes: {}, resolvedIssueCount: 2, createdAt: "2026-08-16T00:00:00.000Z" });

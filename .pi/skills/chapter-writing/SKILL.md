...
5. 组装稿调用 `check_chase_wife_chapter_pacing`、`score_chase_wife_chapter` 和 `check_project_integrity`，再调用 `save_continuity_report` 保存语义审查结果；草稿阶段调用 `check_chase_wife_story_pacing(scope=working)`，全部章节定稿后调用 `scope=finalized`，导出前调用 `finalize_manuscript`。
